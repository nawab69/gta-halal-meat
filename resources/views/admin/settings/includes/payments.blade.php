    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" role="form">
        ...
        <div class="tile-body">
            ...
            <div class="form-group pt-3">
                <label class="control-label" for="bank_pay">Bank Payment</label>
                <textarea
                    class="form-control"
                    rows="10"
                    placeholder="Enter Bank Payment PopUp HTML"
                    id="bank_pay"
                    name="bank_pay"
                >{{ config('settings.bank_pay') }}</textarea>
            </div>
        </div>
...
@push('scripts')
<script>
var simplemde = new SimpleMDE({ element: document.getElementById("bank_pay") });
</script>
@endpush
